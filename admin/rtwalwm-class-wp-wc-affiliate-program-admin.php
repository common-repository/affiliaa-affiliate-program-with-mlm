...
class Rtwalwm_Wp_Wc_Affiliate_Program_Admin {
	...
	public function __construct( $rtwalwm_plugin_name, $rtwalwm_version ) {
		...
		$rtwalwm_generate_aff_csv = isset($_GET['affiliate_csv_download'])? sanitize_text_field($_GET['affiliate_csv_download']): '';
		if(!empty($rtwalwm_generate_aff_csv))
		{
	...
	public function rtwalwm_download_affiliate_csv(){
		...
		header('Content-Disposition: attachment; filename="'.$rtwalwm_file_name.'"');
		$reader = Reader::createFromPath(RTWALWM_DIR.'assets/csv/'.$rtwalwm_file_name, 'r');
		$reader->output();
		wp_die();
	}
...
if( $rtwalwm_column == 'rtwalwm_prod_perc_commission' && current_user_can( 'edit_posts' ) && ($rtwalwm_post_type == 'download' || 'product')){
	...
	*/
	...
	function rtwalwm_generate_csv_affiliate_callback(){
		...
		$rtwalwm_counter = 0;
		require_once( WP_PLUGIN_DIR."/affiliaa-affiliate-program-with-mlm/third_party/csv-9.8.0/autoload.php");
		$rtwalwm_filename 	= "export-labels--".time().".csv";
		$csv = Writer::createFromPath(RTWALWM_DIR.'assets/csv/'.$rtwalwm_filename, 'w+');
		$header = ['S.NO.','Affiliate ID', 'Affiliate Name', 'Affiliate Email','Parent Id','Parent Name' ];
		$csv->insertOne($header);
		if(!empty($rtwalwm_users))
		{
